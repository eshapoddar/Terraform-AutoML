...
// Copy the sample images into the bucket in the new project
resource "null_resource" "copy_images" {
  depends_on = [google_storage_bucket.ml_bucket]
  provisioner "local-exec" {
    command = "gsutil -m cp -R gs://cloud-samples-data/ai-platform/flowers/ gs://${var.bucket_name}/img/"
  }
}

// Create and copy the CSV file into the bucket
resource "null_resource" "create_csv_file" {
  depends_on = [null_resource.copy_images]

  provisioner "local-exec" {
    command = <<-EOT
      # Generate the new CSV content with updated URIs
      cat > all_data.csv <<-EOF
      $(gsutil cat gs://${var.bucket_name}/img/flowers/all_data.csv | sed "s:cloud-ml-data/img/flower_photos/:gs://${var.bucket_name}/img/flowers/:")
      EOF
      # Copy the updated CSV to the bucket
      gsutil cp all_data.csv gs://${var.bucket_name}/csv/
    EOT
  }
}
